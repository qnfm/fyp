...
\coordinate (hs1End) at ($(InitEnd.south)+(0,-3)$);
\coordinate (hs1SEnd) at ($(InitSEnd.south) + (0, -3)$);
% Client and Server: generate secret key
\node[below left, align=right] (processServer) at (InitSEnd.south) {$ss_S \gets KEM_S.Decap(ct_S,sk_S)$\\  $pk_e\gets AEAD_{K_S}.Dec(ct_{pk_e})$\\ ($ss_e,ct_e) \gets KEM_e.Encap(pk_e)$ \\$(ss_C,ct_C)\gets  KEM_C.Encap(pk_C)$\\$K_1\gets KDF(ss_S||ss_e)$};

\draw[-latex,align=center] (hs1SEnd.south)  -- (hs1End.south) node [pos=0.5, above](handshake2) {$ ct_C$ , $ AEAD_{K_1}.Enc(ct_e)$};
\coordinate (hs2End) at ($(InitEnd.south)+(0,-3)$);
\coordinate (hs2SEnd) at ($(InitSEnd.south) + (0, -3)$);

\node[below right, align=left] (processClient) at (hs2End){$ss_C \gets KEM_C.Decap(ct_C,sk_C)$\\$ct_e\gets AEAD_{K_1}.Dec(ct_{ct_e})$\\$ss_e\gets KEM_e.Decap(ct_e,sk_e)$\\$K_1\gets KDF(ss_S||ss_e)$};

\draw[-latex,align=center] ($(hs2SEnd.south) + (0, -3.2)$) -- ($(hs2End.south) + (0, -3.2)$) node [pos=0.5, above]{$K_2,K_2^{'},K_2^{''},K_2^{'''}\gets KDF(ss_S||ss_e||ss_C)$\\$AEAD_{K_2}.Enc$(key confirmation)};
